...
class KvasirFeatureExtractor:
    ...
    def extract_HOG_features(self, image):
        ...
        features['hog_mean'] = np.mean(hog_array)
        features['hog_std'] = np.std(hog_array)
        features['hog_var'] = np.var(hog_array)
        features['hog_max'] = np.max(hog_array)
        features['hog_min'] = np.min(hog_array)
        features['hog_median'] = np.median(hog_array)
        features['hog_skewness'] = self._calculate_skewness(hog_array)
        features['hog_kurtosis'] = self._calculate_kurtosis(hog_array)
 
        percentiles = [1, 5, 10, 25, 75, 90, 95, 99]
        for p in percentiles:
            features[f'hog_p{p}'] = np.percentile(hog_array, p)
 
        features['hog_iqr'] = np.percentile(hog_array, 75) - np.percentile(hog_array, 25)
        features['hog_range'] = np.max(hog_array) - np.min(hog_array)
        features['hog_cv'] = np.std(hog_array) / (np.mean(hog_array) + 1e-8)
 
        features['hog_energy'] = np.sum(hog_array**2)
        features['hog_power'] = np.mean(hog_array**2)
        features['hog_rms'] = np.sqrt(np.mean(hog_array**2))
 
        n_segments = 4
        segment_size = len(hog_array) // n_segments
 
        for i in range(n_segments):
            start_idx = i * segment_size
            end_idx = (i + 1) * segment_size if i < n_segments - 1 else len(hog_array)
            segment = hog_array[start_idx:end_idx]
 
            features[f'hog_q{i+1}_mean'] = np.mean(segment)
            features[f'hog_q{i+1}_max'] = np.max(segment)
            features[f'hog_q{i+1}_std'] = np.std(segment)
            features[f'hog_q{i+1}_energy'] = np.sum(segment**2)
 
        if len(hog_array) > 1:
            gradients = np.diff(hog_array)
            features['hog_grad_mean'] = np.mean(gradients)
            features['hog_grad_std'] = np.std(gradients)
            features['hog_grad_max'] = np.max(np.abs(gradients))
            features['hog_grad_energy'] = np.sum(gradients**2)
 
        mean_val = np.mean(hog_array)
        std_val = np.std(hog_array)
 
        features['hog_peaks_1std'] = np.sum(hog_array > (mean_val + std_val))
        features['hog_peaks_2std'] = np.sum(hog_array > (mean_val + 2*std_val))
        features['hog_valleys_1std'] = np.sum(hog_array < (mean_val - std_val))
 
        features['hog_above_mean_count'] = np.sum(hog_array > mean_val)
        features['hog_above_mean_ratio'] = np.sum(hog_array > mean_val) / len(hog_array)
        features['hog_zero_crossings'] = np.sum(np.diff(np.sign(hog_array - mean_val)) != 0)
 
        sorted_hog = np.sort(hog_array)
        n_extreme = min(10, len(hog_array) // 10)  # Top/bottom 10% ou 10 valores
 
        features['hog_top_extreme_mean'] = np.mean(sorted_hog[-n_extreme:])
        features['hog_bottom_extreme_mean'] = np.mean(sorted_hog[:n_extreme])
        features['hog_extreme_ratio'] = features['hog_top_extreme_mean'] / (features['hog_bottom_extreme_mean'] + 1e-8)
    ...
    def extract_all_features(self, image, mask):
        ...
        features.update(self.extract_HOG_features(image))
