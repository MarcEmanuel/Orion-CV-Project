...
class KvasirFeatureExtractor:
    ...
    def extract_haralick_features(self, image, mask):
    ...
    def extract_morphological_features(self, mask):
        #Extrai características morfológicas do pólipo
        if np.sum(mask) == 0:
            return {}
        ...
        features['aspect_ratio'] = region.major_axis_length / (region.minor_axis_length + 1e-6)
        features['roundness'] = 4 * np.pi * region.area / (region.perimeter ** 2 + 1e-6)
        features['compactness'] = region.perimeter ** 2 / (4 * np.pi * region.area + 1e-6)
    ...
    def extract_intensity_features(self, image, mask):
        ...
        polyp_pixels = image[mask > 0]
        background_pixels = image[mask == 0]
        
        if len(polyp_pixels) == 0:
            return {}
        
        features = {
            'polyp_mean_intensity': np.mean(polyp_pixels),
            'polyp_std_intensity': np.std(polyp_pixels),
            'polyp_median_intensity': np.median(polyp_pixels),
            'polyp_skewness': self._calculate_skewness(polyp_pixels),
            'polyp_kurtosis': self._calculate_kurtosis(polyp_pixels),
        }
        
        # Contraste entre pólipo e fundo
        if len(background_pixels) > 0:
            features['polyp_background_contrast'] = abs(np.mean(polyp_pixels) - np.mean(background_pixels))
            features['intensity_ratio'] = np.mean(polyp_pixels) / (np.mean(background_pixels) + 1e-6)
    ...
    def extract_all_features(self, image, mask):
        ...
        features.update(self.extract_haralick_features(image, mask))
        
        # Características morfológicas (forma)
        features.update(self.extract_morphological_features(mask))
        
        # Características de intensidade (brilho)
        features.update(self.extract_intensity_features(image, mask))
...
if __name__ == "__main__":
    ...
    classifier, results, features_df, labels = main_kvasir_analysis(
        data_path=KVASIR_PATH, 
        max_samples=None  
    )
