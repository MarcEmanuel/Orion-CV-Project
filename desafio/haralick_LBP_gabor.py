...
class KvasirFeatureExtractor:
    ...
    def extract_LPB_features(self, image):
 
        P = 8
        R = 1
 
        lbp = local_binary_pattern(image, P = P, R = R, method='uniform')
        hist, _ = np.histogram(lbp.ravel(),bins=np.arange(0, P + 3), range=(0, P + 2), density=True)  # normalizar para somar 1
        features = {}
 
        features = {f'lbp_hist_{i}': h for i, h in enumerate(hist)}
    ...
    def extract_all_features(self, image, mask):
        ...
        # Caaracteristicas Gabor
        features.update(self.extract_gabor_features(image))
 
 
        # Caracteristivas LPB
        features.update(self.extract_LPB_features(image))
